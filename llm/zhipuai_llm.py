...
class ZhipuAILLM(Self_LLM):
    ...
    model: str = "glm-4"
    """Model name in glm-4, glm-3-turbo. """
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        try:
            if 'data' in response_payload and 'choices' in response_payload['data']:
                return response_payload["data"]["choices"][-1]["content"].strip('"').strip(" ")
            else:
                logger.error(f"Unexpected response structure: {response_payload}")
                return "Error: Unexpected response structure."
        except KeyError as e:
            logger.error(f"Missing key in response: {e}. Response: {response_payload}")
            return "Error: Missing key in response."
